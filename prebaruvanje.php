...
<link rel="icon" href="images/icon.ico.png" type="image/x-icon">
  ...
  <body>
	 ...
	 <span class="create"><img src="images/icon.ico.png" width="100" height="100"/></span>
	      <a class="navbar-brand" href="home.php"> <font size="20" color="white">ТрансБус</font></a>
     ...
     <li class="nav-item"><a href="rezervacija.php" class="nav-link">Мои резервации</a></li>
	          <li class="nav-item"><a href="contact.php" class="nav-link">Контакт</a></li>	    
...
extract($_POST);
$poagjanje=$_POST['poaganje'];
$destinacija=$_POST['destinacija'];
        $data=$_POST['data'];
$con=mysqli_connect("localhost","root","","trans");
$query= "select * from ruta where poagjanje='$poagjanje' AND destinacija='$destinacija'";
...
echo "Број на пронајдени рути:     ".$num_results;
echo"<br>";
echo"<br>";
        ?>  </h3>  Датум за избраната автобуска линија:      
...
echo "<td>" . $row['cena'] . "</td>";
    ...
    $row['ID_ruta'];
    $idruta = $row['ID_ruta'];
    $niza=[$row['ID_ruta'], $data];
    echo "<td>".'<button type="submit" method="post" action="rezervacija.php" name="ruta" value="'.$idruta.'">Резервирај</button>'. "</td>";
echo "</tr>";
}
          echo $data;
